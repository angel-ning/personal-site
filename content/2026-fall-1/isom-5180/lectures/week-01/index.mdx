...
<Figure src="images/page_59.png" caption="Dedicated paths / microsegmentation — Before：共享网段上所有流量对所有人可见；After：交换机内部给每对主机独立通路" source="Slide 59" />
...
<Figure board src="images/BOARD_ROUTERS.png" caption="三台路由器 A、B、C：以太网接口 E0/E1 连 PC，串行接口 S0/S1 连其他路由器；路由表 = IP → Interface；路由协议 RIP / EIGRP / OSPF" source="M1 Supplementary p.6" />
...
<Callout type="warn" title="踩坑提醒：Dynamic 学习和交换机的学法「不一样」">
...
<Callout type="note" title="TODO 已解决（见第 2 周笔记）">
...
<Callout type="extra" title="课外补充：Default route 与最长匹配">
...
<Callout type="warn" title="踩坑提醒：写自己的出口，不是对方的端口">
...
<Callout type="warn" title="踩坑提醒：「全 1」不是把地址加起来算出来的">
...
<QA q="11. MAC address table 为什么需要 Time stamp？如果没有会出什么问题？">
...
<QA q="12. 交换机有哪些防止 collision 的机制？分别在交换机的哪里起作用？">
...
<QA q="13. 比较 MAC address table 和 routing table：层、记录内容、如何建立、查不到时怎么办。">
...
<QA q="14. 路由器的 Serial 接口和 Ethernet 接口有什么不同？为什么说路由器能连接不同的 Layer 2 技术？">
...
<QA q="15. 用板书拓扑（A–S0–S0–B–S1–S0–C，每台路由器 E0、E1 各接一个网络），写出 Router C 的 Initial R.T. 和完整 R.T.">
...
<QA q="16. 一个 MAC 地址有多少位？OUI 是什么？MAC 广播地址和 IPv4 广播地址分别是什么？">
...
<QA q="17. 在「Data Flow Through a Network」中，数据从 Fred 经过 SW1、R1、H1 到达 Barney。每个设备分别处理到第几层？">
