...
<Callout type="note" title="本笔记里的几种标记">
...
<Figure src="images/page_03.png" caption="Bit / Byte / KB / MB / GB / TB 对照表" source="Slide 3" />
...
<Callout type="extra" title="课外补充：小写 b 和大写 B 差 8 倍">
...
<Figure board src="images/BOARD_CODE_DECODE.png" caption="Code / Decode：I miss you → 1 2 1 3 3 4 5 6 → I miss you" source="M1 Supplementary p.1" />

<Callout type="board" title="老师板书：编码 (Code) 与解码 (Decode)">
...
<Figure src="images/page_04.png" caption="Devices · Media · Services" source="Slide 4" />
...
<Figure src="images/page_09.png" caption="拓扑图里的终端、中间设备、介质符号：波浪线 = 无线，直线 = LAN，红色闪电线 = WAN" source="Slide 9" />
...
<Figure board src="images/BOARD_PORT_INTERFACE.png" caption="设备上的一排口：Port / Interface" source="M1 Supplementary p.3" />
...
<Figure src="images/page_12.png" caption="LAN 与 WAN 的定义与对比" source="Slide 12" />
...
<Figure src="images/page_15.png" caption="同心圆：Intranet（公司内部）⊂ Extranet（供应商、客户、合作方）⊂ Internet（全世界）" source="Slide 15" />
...
<Figure src="images/page_17.png" caption="两类网络安全 + 三个安全目标" source="Slide 17" />
...
<Figure src="images/page_19.png" caption="外部威胁穿过防火墙，内部威胁直接从里面发起" source="Slide 19" />
...
<Figure board src="images/BOARD_ACL.png" caption="ACL 例子：一台路由器 R 连着老师 PC1、学生 PC2、5180 考试服务器和邮件服务器" source="M1 Supplementary p.2" />

<Callout type="board" title="老师板书：ACL 是什么">
...
<Figure src="images/page_24.png" caption="Bandwidth Highway Analogy：带宽 = 车道数，网络设备 = 匝道、红绿灯、路牌、地图，数据包 = 车" source="Slide 24" />
...
<Callout type="extra" title="课外补充：带宽 ≠ 实际速度">
...
<Figure src="images/page_28.png" caption="OSI Reference Model 七层与六个好处" source="Slide 28" />
...
<Callout type="extra" title="课外补充：TCP vs UDP（Slide 32 只点了名字）">
...
<Figure board src="images/BOARD_MAC_VS_IP.png" caption="NIC → MAC（Physical）；HK · UST · ISOM 这种分级地址 → IP（Logical）；Find the path = Path determination" source="M1 Supplementary p.8" />

<Callout type="board" title="老师板书：名字 vs 地址">
...
<Figure src="images/page_37.png" caption="TCP/IP 四层与各层常见协议" source="Slide 37" />
...
<Figure src="images/page_39.png" caption="数据从 Source 往下走：每层加上自己的 header（数据链路层还加 trailer）" source="Slide 39" />
...
<Figure src="images/page_40.png" caption="Names for Data at Each Layer" source="Slide 40" />
...
<Figure board src="images/BOARD_ENCAP_FCS.png" caption="Sender S (IP-1, MAC-1) → Receiver R (IP-2, MAC-2)：逐层封装 + FCS 校验" source="M1 Supplementary p.9" />

<Callout type="board" title="老师板书：一份数据是怎么被打包的">
...
<Figure board src="images/BOARD_BUS_RING.png" caption="Bus（两端有终结器，信号到头不反弹）与 Ring（令牌 Token 沿环传递）" source="M1 Supplementary p.2" />

<Figure board src="images/BOARD_FULL_MESH.png" caption="Full mesh：每两台设备之间都有一条线" source="M1 Supplementary p.3" />
...
<Figure src="images/page_43.png" caption="课件里的设备图标：Repeater、Hub、Bridge、Workgroup Switch、Router、Network Cloud" source="Slide 43" />

### 🟡 Repeater 中继器 — Layer 1（Slide 44–47）

<Figure src="images/page_44.png" caption="Betty 发出干净信号 ① → 到中继器时已经变形 ② → 中继器重新生成干净信号 ③" source="Slide 44" />
...
<Figure src="images/page_49.png" caption="Hub Operation：「我在 0 号口收到信号——从其他所有口发出去！」" source="Slide 49" />

<Figure board src="images/BOARD_SHARED_BUS.png" caption="共享介质上：PC1 → PC3 时每台 PC 都会收到并复制这一帧；PC1 和 PC2 同时发送就会冲突" source="M1 Supplementary p.10" />

<Callout type="board" title="老师板书：共享介质上发生了什么">
...
<Callout type="extra" title="课外补充：CSMA/CD 与冲突域">
...
<Figure src="images/page_50.png" caption="Switches filter traffic by looking at MAC addresses" source="Slide 50" />
...
<Figure board src="images/BOARD_ROUTERS.png" caption="三台路由器 A、B、C：以太网接口 E0/E1 连 PC，串行接口 S0/S1 连其他路由器；路由表 = IP → Interface" source="M1 Supplementary p.6" />

<Callout type="board" title="老师板书：路由器的接口与路由表">
...
<Figure src="images/page_53.png" caption="Flooding Unknown Unicasts：CAM 表一开始是空的 → Fred 从 1 号口发帧 → 交换机记下 0200.1111.1111 在 1 号口 → 不知道 Barney 在哪，只能泛洪" source="Slide 53" />

<Figure src="images/page_55.png" caption="两台交换机：SW1 学到 Fred 在 1 号口；SW2 学到 Fred 在 3 号口（从 SW1 那条线进来的）" source="Slide 55" />
...
<Figure src="images/page_56.png" caption="Filter：Fred 发给 Barney，两人都在 0 号口那一侧 → 交换机不转发" source="Slide 56" />

<Figure src="images/page_57.png" caption="Forward：Fred 发给 Wilma，Wilma 在 1 号口 → 只从 1 号口转发" source="Slide 57" />

### 🔴 老师板书：完整走一遍 MAC 表

<Figure board src="images/BOARD_MAC_TABLE.png" caption="交换机四个口 E0–E3；F、L 在 E0 那一侧，Q、V 在 E1 那一侧；MAC 表含 Time stamp，5 分钟老化" source="M1 Supplementary p.4" />
...
<Callout type="extra" title="课外补充：MAC flooding 攻击（安全课会回来讲）">
...
<Figure src="images/page_59.png" caption="Before：共享网段上所有流量对所有人可见；After：交换机内部给每对主机独立通路" source="Slide 59" />
...
<Figure board src="images/BOARD_COLLISION_SWITCH.png" caption="左：Bus / Hub 上 PC1→PC3 和 PC2→PC4 同时发会冲突；右：Switch 内部有独立通路；下：全双工双车道 与 缓冲红绿灯" source="M1 Supplementary p.5" />

<Callout type="board" title="老师板书：Bus / Hub vs Switch">
...
<Figure src="images/page_60.png" caption="Switch Buffering Example：Wilma 发给 Fred 的同时，Barney 和 Betty 也发给 Fred → 先存进 Frame Buffers，再逐个转发" source="Slide 60" />
...
<Figure src="images/page_66.png" caption="Switch Modes 页：交换机表记录 Interface ↔ MAC Address（E0 后面两台，E1 后面两台）" source="Slide 66" />

<Callout type="extra" title="课外补充：交换机的三种转发模式 (Switching Modes)">
...
<Figure src="images/page_63.png" caption="The Ethernet MAC Address Structure：OUI (24 bits) + Vendor Assigned (24 bits)" source="Slide 63" />
...
<Figure board src="images/BOARD_BROADCAST.png" caption="广播：目的地址不是把每个 IP 列出来，而是一个「全 1」的特殊地址" source="M1 Supplementary p.7" />

<Callout type="board" title="老师板书：广播地址长什么样">
...
<Figure src="images/page_68.png" caption="Fred → SW1 → R1 → H1 → Barney：每台设备只拆到自己那一层" source="Slide 68" />
...
<QA q="6. 按从上到下写出 OSI 七层，并写出 L4、L3、L2 的 PDU 名称和使用的地址 / 标识。">
...
<QA q="12. 在「Data Flow Through a Network」中，数据从 Fred 经过 SW1、R1、H1 到达 Barney。每个设备分别处理到第几层？">
