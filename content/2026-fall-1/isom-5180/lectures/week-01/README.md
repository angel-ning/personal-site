...
> **📝 TODO：课后问老师**
>
> - 课件和板书对 dynamic routing 只**列了名字**（RIP、EIGRP、OSPF），没有展开协议具体怎么交换路由信息。课上是不是只要求会**画表**（就是下面的 Initial R.T. → 完整 R.T.），不考 dynamic 协议的细节？
> - 考试画路由表时，**路由器之间串行线本身的网络**（A–B、B–C 那两段）要不要也写进表里？
> - IP address 分成 **network 部分和 host 部分**、子网掩码——老师说后面会展开，到时候回来把下面例子里的地址补成完整写法。
