...
> 🟢 **了解即可** — 背景知识，考试大概率不会细抠
>
> **优先级是我推断的**（课件没写考试形式，你也还没告诉我老师强调了什么，可以随时改）：  
> ① **篇幅**：交换机相关内容占了 p.43–67 共 25 页，OSI 七层占 p.28–41 共 14 页，是本课两大块；  
> ② **老师的课堂手写补充（M1 Supplementary）**：10 页里有 6 页在画「帧怎么走」——MAC 表、封装、FCS、冲突、广播地址。老师愿意现场画的，通常就是他要考的；  
...
> - 网页版里有两个可以点的小实验（交换机 MAC 表、FCS 差错检测），打印版会变成等价的表格。
...
数据的单位 (bit / byte)
   ...
   → 每一层有自己的设备：Hub = L1 · Switch = L2 · Router = L3
   → 交换机的核心动作：学 MAC 表 → Flood / Forward / Filter
   → MAC 地址长什么样 · 单播 / 广播 / 组播
...
## 11. 🔴 Networking Devices（按层区分的网络设备）
...
>
> → 所以 Hub / Bus 上**所有设备处在同一个 collision domain（冲突域）**，设备越多冲突越频繁。这正是交换机要解决的问题。
...
### 🔴 Switch 交换机 — Layer 2（Slide 50–51）
...
- 收到帧后，查**目的 MAC** 决定 **filter, flood or forward**（详见第 12 节）

### 🔴 Router 路由器 — Layer 3（Slide 62）

- **Layer 3 device**；**Use logical address (network layer address)**，即 IP 地址
- **Can connect different layer 2 technologies**（例如一边是以太网、一边是串行 WAN 线路）
- **Examine incoming packet (layer 3 data), choose the best path, and switch them to the proper outgoing port**

![三台路由器 A、B、C：以太网接口 E0/E1 连 PC，串行接口 S0/S1 连其他路由器；路由表 = IP → Interface](images/BOARD_ROUTERS.png)

*✍️ 三台路由器 A、B、C：以太网接口 E0/E1 连 PC，串行接口 S0/S1 连其他路由器；路由表 = IP → Interface（M1 Supplementary p.6）*

> **✍️ 老师板书：路由器的接口与路由表**
>
> - 每台路由器有好几个 **interface**：**E0、E1 = Ethernet** 接口（连本地 PC），**S0、S1 = Serial** 接口（路由器之间的 **serial connection**，通常是 WAN 链路）。例如 A 的 S0 连 B 的 S0，B 的 S1 连 C 的 S0。
> - **Routing table 路由表** 只有两列核心信息：**目的网络 (IP) → 从哪个 Interface 发出去**。
> - 路由表可以手工写，也可以由 **routing protocol（路由协议）** 自动学习和更新：**RIP、EIGRP、OSPF**（都在 TCP/IP 图的 Internet 层里）。
> - 板书右下角是拓扑图常用符号：**Media（线）、PC、Network Cloud（云 = 不关心内部细节的一片网络，例如 Internet 或运营商 WAN）**。
...
| 怎么转发  | 从**所有其他端口**发出去     | 查 **MAC 表**：flood / forward / filter | 查**路由表**：选最佳路径        |
...
## 12. 🔴 How a Switch Works：Learn → Flood / Forward / Filter

这是本周篇幅最大的一块（Slide 51–67），也是老师板书最详细的地方。
...
**Time stamp 与 5 分钟老化 (aging)**：每条记录都有时间戳，每次再看到这个源 MAC 就刷新；**超过 5 分钟没再出现就删掉**（Cisco 交换机默认 300 秒）。板书里 MAC-Q 被划掉，是在演示老化：主机关机、挪走或长时间不发帧，它的记录就会被删除，MAC 表不会一直保留过期信息（下面的小实验可以点「过 5 分钟」看效果）。
...
## 13. 🟡 LAN Switch 的好处：微分段、全双工、缓冲

![Before：共享网段上所有流量对所有人可见；After：交换机内部给每对主机独立通路](images/page_59.png)

*Before：共享网段上所有流量对所有人可见；After：交换机内部给每对主机独立通路（Slide 59）*

**LAN Switches**（Slide 58）：

- **Layer 2 device**；用 **table of MAC addresses (switching table)** 决定帧该发往哪个网段
- Cisco 把 switching table 称为 **CAM (Content Addressable Memory)** table
...
> 2. **Switch**：PC1→PC3 走 E0→E2，PC2→PC4 走 E1→E3，交换机内部是**不同的通路**，互不干扰 → **没有冲突**。
> 3. **Full duplex（全双工）= send and receive at the same time**：像双向车道，PC1→PC3 和 PC3→PC1 可以同时进行。（Hub 只能 half duplex，同一时间只能一个方向。）
> 4. 真正会「撞」的情况：**PC1→PC3 和 PC2→PC3 同时发给同一台**——两条车道并入一条。交换机的办法是像红绿灯一样**先把一帧存起来 (buffering)**，一帧一帧地放行。
> 5. 交换机 E4 口接上一台 **Router**，Router 后面是 PC5–PC8 的另一个网络：跨网络就要靠路由器。
...
**Preventing Collisions with Switch Buffering**（Slide 61）：
...
| 类型               | 课件定义                                                                                               | 发送方 : 接收方 | 例子                                    |
| ---------------- | -------------------------------------------------------------------------------------------------- | --------- | ------------------------------------- |
| **Unicast 单播**   | A frame is sent from one host addressed to **a specific destination**                              | 1 : 1     | 访问网页、发邮件；**LAN 和 Internet 上最主要的传输方式** |
...
>
> - ✗ 不是写成 `IP (S, D) = (IP-1, IP-2 + IP-3 + IP-4 + IP-5)`（老师打了红叉）
> - ✓ 而是 `IP (S, D) = (IP-1, 255.255.255.255)`
>
> **IPv4 broadcast address**：IPv4 地址 32 位**全部是 1** → 每 8 位一组 `11111111` = 255 → **255.255.255.255**  
> **MAC broadcast address**：48 位**全部是 1** → 每 4 位 `1111` = F → **12 个 F**：`FF-FF-FF-FF-FF-FF`  
...
**11. 一个 MAC 地址有多少位？OUI 是什么？MAC 广播地址和 IPv4 广播地址分别是什么？**

> 48 bits，写成 12 个十六进制数。**OUI (Organizationally Unique Identifier)** 是前 3 字节（24 bits），由 IEEE 分配给厂商；后 3 字节由厂商保证唯一。MAC 广播地址 = **FF-FF-FF-FF-FF-FF**（48 个 1）；IPv4 广播地址 = **255.255.255.255**（32 个 1）。

**12. 在「Data Flow Through a Network」中，数据从 Fred 经过 SW1、R1、H1 到达 Barney。每个设备分别处理到第几层？**
