...
**实际网络里，路由表通常由 dynamic routing protocol 完成**（网络一大，手工写每一条既累又容易错，线路断了也不会自动绕路）；static 多用在小网络或固定的出口（比如默认路由）。

> **📌 TODO 已解决（见第 2 周笔记）**
>
> - **Dynamic routing 讲不讲细节？** —— 讲。Module 2 的 Slide 52–66 专门讲 routing protocol（RIP 怎么通告路由、metric 怎么选路）；第 2 周的板书也画了「A 把 1.0、2.0、3.0 告诉 B，B 把 3.0、4.0、5.0 告诉 A」。
> - **路由器之间串行线本身的网络要不要写进表？** —— **要**，两边都写成直连网络（课堂板书的 3.0，以及 Slide 55、64）。
> - **IP 分 network / host** —— 第 2 周第 1–3 节。
...
### 🔴 考法：画图 → 标端口 → Initial R.T. → 完整 R.T.

这是本节最该练的题型：给一张路由器拓扑图，先把**每个端口标出来**，写出每台路由器的 **Initial routing table**，再补全成完整的路由表。用板书的三台路由器练一遍。

**Step 1 — 标端口、给每个网络一个地址。** 板书连法：A 的 **S0 ↔** B 的 **S0**，B 的 **S1 ↔** C 的 **S0**；每台路由器的 E0、E1 各接一个 PC 所在的网络。为了能写表，给这些网络编上（假设的）地址：

| 路由器   | E0            | E1            | S0       | S1       |
| ----- | ------------- | ------------- | -------- | -------- |
| **A** | 10.1.1.0（PC1） | 10.1.2.0（PC2） | → B 的 S0 | —        |
| **B** | 10.2.3.0（PC3） | 10.2.4.0（PC4） | → A 的 S0 | → C 的 S0 |
| **C** | 10.3.5.0（PC5） | 10.3.6.0（PC6） | → B 的 S1 | —        |

**Step 2 — Initial R.T.：只有直连网络。** 路由器刚开机、还没配置 static、也还没跑路由协议时，表里只有**自己接口直接连着的网络**，路由器自动写进去：

| Router A |               | Router B |               | Router C |               |
| -------- | ------------- | -------- | ------------- | -------- | ------------- |
| **IP**   | **Interface** | **IP**   | **Interface** | **IP**   | **Interface** |
| 10.1.1.0 | E0            | 10.2.3.0 | E0            | 10.3.5.0 | E0            |
| 10.1.2.0 | E1            | 10.2.4.0 | E1            | 10.3.6.0 | E1            |

这时 A 只知道 PC1、PC2 两个网络；PC1 想发给 PC5，A 查不到 → **Discard**。

**Step 3 — 完整 R.T.：补上远程网络。** 远程网络要么由管理员**手工写 (static)**，要么由路由协议**自动学 (dynamic)**：B 通过 S0 告诉 A「10.2.3.0、10.2.4.0 在我这儿，10.3.x 我也能到」，A 就把这些网络记成「从 **S0** 出去」。结果：
...
| 10.1.1.0 | E0            | 10.2.3.0 | E0            | 10.3.5.0 | E0            |
| 10.1.2.0 | E1            | 10.2.4.0 | E1            | 10.3.6.0 | E1            |
| 10.2.3.0 | S0            | 10.1.1.0 | S0            | 10.1.1.0 | S0            |
| 10.2.4.0 | S0            | 10.1.2.0 | S0            | 10.1.2.0 | S0            |
| 10.3.5.0 | S0            | 10.3.5.0 | S1            | 10.2.3.0 | S0            |
| 10.3.6.0 | S0            | 10.3.6.0 | S1            | 10.2.4.0 | S0            |

**规律**：直连网络 → 写连着它的那个 **E 口**；远程网络 → 写**朝那个方向出去的、本路由器自己的端口**（这里都是 Serial 口）。A 只有一条 S0 通往外面，所以所有远程网络都写 S0；B 在中间，往 A 那边的写 S0，往 C 那边的写 S1。

> **⚠️ 踩坑提醒：写自己的出口，不是对方的端口**
>
> C 去 PC1、PC2 的网络，要写 **C 自己的 S0**，不是对面 B 的 S1。路由表回答的问题是「**我**该把包从**我的哪个口**送出去」。（真实路由表里还会多写一列 **next hop**，也就是对面那台路由器的 IP；本课的简化画法只写 IP | Interface。）

**走一遍 PC1 → PC5**：A 查表 10.3.5.0 → S0 → 送到 B；B 查表 → S1 → 送到 C；C 查表 10.3.5.0 是直连 E0 → 交给 PC5。**PC1 → 99.9.9.9**：A 的表里没有这个网络（也没有 default route）→ **Discard**。
...
**15. 用板书拓扑（A–S0–S0–B–S1–S0–C，每台路由器 E0、E1 各接一个网络），写出 Router C 的 Initial R.T. 和完整 R.T.**

> Initial R.T. 只有直连：10.3.5.0 → E0，10.3.6.0 → E1。完整 R.T. 再加上所有远程网络，它们都在 B 那一侧，所以都写 **C 自己的 S0**：10.1.1.0、10.1.2.0、10.2.3.0、10.2.4.0 → S0。（远程条目可以由管理员手工配置 static，也可以由 RIP / EIGRP / OSPF 动态学到。）

**16. 一个 MAC 地址有多少位？OUI 是什么？MAC 广播地址和 IPv4 广播地址分别是什么？**
...
**17. 在「Data Flow Through a Network」中，数据从 Fred 经过 SW1、R1、H1 到达 Barney。每个设备分别处理到第几层？**
