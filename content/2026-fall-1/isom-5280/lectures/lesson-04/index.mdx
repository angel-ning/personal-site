...
<Figure src="images/page_03.png" caption="Scytale：公元前 487 年斯巴达人用的加密棒" source="Slide 3" />
...
<Callout type="exam" title="Class Discussion（p.18）：对称加密的根本局限是什么？">
...
<Figure src="images/page_13.png" caption="Public Key Encryption 示例：Alex 用 Rachel 的公钥加密，只有 Rachel 的私钥能解开" source="Slide 13" />
...
<Figure src="images/page_17.png" caption="HTTPS 的 TLS/SSL 握手：Client Hello/Server Hello → Certificate → 非对称加密协商 → Key Exchange → 对称加密传输" source="Slide 17" />
...
<MCQ q="True or False: Hashing functions require the use of keys?" a="True" b="False" answer="B">
...
<Figure src="images/pki_02.png" caption="Hash as Message Digest：Alice 把消息和它的哈希一起发给 Bob，Bob 重新哈希消息再比对" source="PKI Slide 15" />
...
<Figure src="images/pki_03.png" caption="Digital Signature of Sender：Alice 把摘要用自己的私钥加密成签名；Bob 用 Alice 的公钥解开签名，拿到原始摘要再比对" source="PKI Slide 16" />
...
<Figure src="images/pki_05.png" caption="Digital Certificate：Certificate Authority 为 Alice 的公钥背书，Bob 收到的不再是裸公钥而是一张证书" source="PKI Slide 18" />
...
<Figure src="images/page_26.png" caption="Digital Certificate Sample：General / Details / Certification Path 三个标签页——签发对象、签发者、有效期、证书链一路指向根 CA" source="Slide 26" />
...
<QA q="PKI（公钥基础设施）到底给一个组织带来了什么价值？（p.27 Class Discussion）">
...
<Callout type="warn" title="蓝牙安全提醒（p.16 PKI 课件）">
...
<Figure src="images/page_33.png" caption="Blockchain Data Integrity：每个 Block 把上一个 Block 的哈希值（Prev Hash）存进自己里面，串成一条哈希链" source="Slide 33" />
...
<Figure src="images/page_38.png" caption="Quantum Computing – Not a 2036 Problem：金融业量子计算采用率仅 7%，KPMG 建议现在就做 Quantum Preparedness Index 评估" source="Slide 38" />
...
<MCQ q="Which of the following terms is used to describe the information used in conjunction with an algorithm to create the ciphertext from the plaintext?" a="Cipher" b="Code" c="Clear text" d="Key" answer="D">
...
<MCQ q="What term is used to describe a cryptographic method that incorporates mathematical operations involving both a public key and a private key to encipher or decipher a message?" a="Private-key encryption" b="Symmetric encryption" c="Advanced Encryption Standard (AES)" d="Asymmetric encryption" answer="D">
公钥 + 私钥同时参与运算，正是 Asymmetric Encryption（非对称加密）的定义；A/B/C 都是对称加密的说法或具体算法。
</MCQ>

<QA q="为什么说「Digital Certificate 解决的是 Digital Signature 解决不了的问题」？">
...
<QA q="HTTPS 里的 TLS 握手，为什么不全程使用非对称加密，也不一开始就用对称加密？">
