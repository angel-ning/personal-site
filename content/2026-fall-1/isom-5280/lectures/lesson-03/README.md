...
title:
  ...
  zh: "第 3 课 · 安全技术与防护工具"
summary:
  en: "Access control and ZTA, the full firewall stack (packet-filtering → stateful → proxy) and architectures, VPN, IDPS terminology and detection methods, EDR/SIEM/NGFW/SOC/XDR/SOAR, honeypots, and the security-controls summary."
  zh: "访问控制与零信任、完整防火墙谱系（包过滤→状态检测→代理）与架构、VPN、IDPS 术语与检测方法、EDR/SIEM/NGFW/SOC/XDR/SOAR、蜜罐，以及安全控制策略总结。"
week: 3
date: 2026-09-07
tags: [Firewall, IDPS, Zero Trust, SIEM, SOC, XDR, SOAR, Honeypot]
---
...
> "前一层的 \_\_\_ 有 \_\_\_ 这个固有局限（说轴一或轴二的短板），因此需要 \_\_\_ 来补足，代价是 \_\_\_（性能/成本/复杂度），这体现了 defense-in-depth 的思想。"
...
下面这个互动组件把这张规则表变成可以自己喂包进去测试的工具——自己填 Source IP / Dest IP / Dest Port，看它命中第几条规则、结果是 Allow 还是 Deny：
...
**Quick Recap #1 (p.22): \_\_\_\_\_ inspection firewalls keep track of each network connection between internal and external systems using a state table.**
...
> **答案：C**
>
> 定义直接对应 p.19。⚠️ 干扰项 **b. Dynamic** 很强，因为课件原文就有 "**dynamic** state table"——但那是形容 table 的词，**技术本身叫 Stateful Inspection**。

**Multiple Choice #1 (p.63): Under MFA, a system that requires users to enter a passcode and then verifies that their face matches a photo stored in the system. What 2 factors is it using?**
...
> **答案：D**
>
> passcode = Something you **know**；face = Something you **are**。⚠️ **A 和 B 是同一个答案换顺序 → 必定都不是答案**，直接在 C、D 里选。

**Multiple Choice #2 (p.64): Incident originating from a single system on the Internet targeting multiple systems on this network. What control could stop the incident ASAP?**
...
> **答案：D**
>
> 单一源（不是 **D**istributed → 排除 A）；多个目标（逐台配主机规则太慢 → 排除 B）；漏洞评估不能阻止正在发生的事（排除 C）。**在网络边界加一条规则，一次保护所有内网系统。**

**Multiple Choice #3 (p.65): NIDS alerted to a DDoS attack, and investigation revealed that this type of attack did take place. What type of report?**
...
> **答案：C**
>
> 系统**报了**（Positive）+ 事实**确实发生**（True）= **True Positive**。
...
> 三类：**Something You Know**（密码、安全问题）、**Something You Have**（Token、手机短信、ID 卡）、**Something You Are**（指纹、人脸、声纹）。"密码 + 安全问题"两个都属于 Something You Know——**必须来自不同类别才算真正的 MFA**，这只是 2 步验证，不是多因素。

**2. 画出 Access Control Approaches 的完整树状图，并说明 Mandatory 和 Role-based 分别在树的哪个位置。（→ §6）**

> 树：Access Control 分 **Discretionary**（用户自己决定）和 **Nondiscretionary**（组织决定）；Nondiscretionary 之下是 **Lattice-based**，再分出 **Mandatory**（系统按密级标签强制裁决）和 **Role-based/Task-based**（按角色/任务授权）。**Mandatory 和 Role-based 都在 Nondiscretionary → Lattice-based 之下，和 Discretionary 不是同一层**，不能把三者拍平成并列关系。

**3. ZTA 的三大原则是什么？其中「Assume Breach」为什么会改变整个安全架构的设计思路？（→ §7）**

> 三大原则：**Never Trust, Always Verify**（永不信任，始终验证）、**Least Privilege Access**（最小权限访问）、**Assume Breach / Continuous Verification**（假定已被攻陷/持续验证）。"Assume Breach" 之所以能改变设计思路，是因为**一旦假设攻击者已经在内部**，设计重点就会从"堆预防"转向"做微分段、持续验证、重视检测"——这正是本课后半段 IDPS/SIEM/SOC 存在的逻辑起点。

**4. Stateless 防火墙「无法知道」的三件事是什么（课件原文）？这个短板具体导致了 p.18 规则表里的哪一条规则？（→ §11, §12）**

> 三件事：**是否属于已有连接、是否在尝试建立新连接、还是一个野生乱入包**。这个短板导致规则表**第 1 条**——为了让回包能进来，只能把内网所有 >1023 的高位端口对全世界永久敞开，攻击者伪造源 IP/端口即可随时塞包进来。

**5. 给出的规则表中，第 6、7 条（Telnet）如果顺序互换会发生什么？为什么？（→ §12）**

> 规则表是**自上而下逐条匹配、一旦匹配就不再往下看**。如果把"内网之间允许 Telnet"（第 6 条）和"外部一律拒绝 Telnet"（第 7 条）顺序互换，所有 Telnet 流量（包括内网之间的）都会先命中"一律拒绝"这条，导致**内网之间的 Telnet 也被误杀**。

**6. Stateful 防火墙「只接受匹配 state table 的流量」——那新连接还怎么建立？完整描述它的两步判断逻辑。（→ §13）**

> "只接受匹配 state table 的流量"这句话**只管入方向**。真正逻辑分两步：① 包进来先查 state table，属于已记录的连接就直接放行；② 不属于任何已有连接（=新连接请求）就去查 firewall ruleset，规则允许则放行**并在 state table 里新建一条记录**，不允许则丢弃。

**7. 从 Traffic Filtering / Context Awareness / Resources Usage / Performance Impact 四个维度对比 Stateless 和 Stateful。（→ §14）**

> **Traffic Filtering**：Stateless 逐包独立看包头+静态规则；Stateful 以连接为单位，看包头+内容+state table。**Context Awareness**：Stateless 无上下文；Stateful 知道每条连接的状态。**Resources Usage**：Stateless 低；Stateful 高（要维护动态 state table）。**Performance Impact**：Stateless 小（快）；Stateful 较大（高并发可能成瓶颈）。整体上 Stateful 安全性远强于 Stateless，代价是资源和性能开销。

**8. Application Layer Proxy Firewall 与前两种防火墙最本质的机制区别是什么？这个区别带来了哪四层防护能力？（→ §15）**

> 最本质区别：**连接被切成两段**——包过滤/状态检测是"转发"（包原封不动到达服务器），Proxy 是"代理"（连接在 proxy 处终止，再由 proxy 以自己的身份重新发起）。**没有任何外部数据包真正进入内网**。四层防护能力：① 架构性隔离（畸形包/协议漏洞打到 proxy 就停）；② 天然 default-deny（没有对应模块的协议过不去）；③ 内容级过滤（能封 URL、禁止特定文件类型）；④ 完整审计（记录谁访问了什么）。

**9. 把 Packet-filtering / SPI / Application proxy / MAC layer firewall 四种防火墙对应到 OSI 层。（→ §16）**

> **MAC layer firewall** → L1–L2（数据链路层，按 MAC 地址过滤）；**Packet-filtering firewall** → L3（网络层，只看 IP）；**SPI (Stateful) firewall** → L3–L4（同时看 IP 和 TCP 端口/连接状态）；**Application layer proxy firewall** → L5–L7（看应用层内容）。层次越高看得越深越安全，但越慢越贵。

**10. Screened Host 架构中，为什么要在 proxy 前面加一个 packet-filtering router？请给出两个理由。（→ §17）**

> **理由 A（安全/defense-in-depth）**：攻击者要过两关，且两关机制不同（一个看包头、一个看内容），绕过 router 未必绕得过 proxy。**理由 B（性能）**：课件明写 "lessening the proxy server's load"——proxy 检查得深但慢，让 router 先用便宜快速的方式挡掉大部分明显垃圾流量，减轻 proxy 的负担。

**11. Screened Host 与 Screened Subnet 的核心区别是什么？后者解决了前者的什么致命弱点？（→ §18）**

> Screened Host 只有一道 router + 一台 bastion host，且**bastion host 本身还连在内网上**——一旦它被攻陷，攻击者就直接站在内网里了。Screened Subnet（DMZ）用**两个过滤路由器**把 DMZ 夹在中间，形成三段式架构：外部只能到 DMZ，DMZ 到内网还要再经过一次代理访问（Proxy access），**外部与内网之间从来没有直连规则**，解决了 Screened Host 里 bastion host 直连内网的致命弱点。

**12. VPN 的 Tunnel Mode 和 Transport Mode 分别加密什么？课件说 Remote Access to Office 属于哪一种？（→ §20）**

> **Tunnel Mode** 加密整个 IP 包（包括包头），外层重新封装一个新 IP 头，常用于站点到站点。**Transport Mode** 只加密 payload，原 IP 头保持不加密。课件明确说 **Remote Access to Office 属于 Transport Mode**（按课件答，不要用业界惯例"纠正"它）。

**13. 课件说「VPN must accomplish (CIA)」，这里的 A 是什么？和 Lec 1 的 CIA 有何不同？（→ §20）**

> 这里的 A 是 **Authentication（身份验证）**，不是 Lec 1 CIA 三要素里的 Availability。因为 VPN 走公共互联网，没法保证可用性（运营商断网它就断），但能保证内容不被看到（Confidentiality，靠加密）、不被改（Integrity，靠封装）、以及只有对的人能用（Authentication，靠密码/密钥/数字签名）。

**14. 写出 S = P + D + R 公式，并各举本课两个工具。IDS 存在的根本理由是什么？（→ §21）**

> **P（Prevention）**：防火墙、VPN；**D（Detection）**：IDS、SIEM；**R（Response）**：IPS、SOAR。IDS 存在的根本理由：**预防一定会失败**（0-day、内鬼、社工、配置错误），光有 P 不叫安全，必须承认"防火墙一定会被绕过"，靠 D 去发现绕过之后的行为——这和 ZTA 的 Assume Breach 是同一个思想。

**15. Intrusion 和 Incident 有什么区别？举一个「是 incident 但不是 intrusion」的例子。（→ §21）**

> **Intrusion（入侵）** 是一次未授权访问的尝试或事件，即使没成功也算；**Incident（安全事件）** 是真正违反安全策略、造成或可能造成实际影响的事。"是 incident 但不是 intrusion"的例子：**员工误删数据库**、笔记本遗失、配置错误导致数据泄露——这些造成了实际影响，但都不是"外部未授权访问的尝试"。

**16. IDPS Benefits 的三条分别面向哪三类受众、对应哪三个时间点？（→ §22）**

> ① **Intrusion Detection**（事中/实时）→ 面向**安全运维团队**；② **Data Collection & Forensic**（事后/回溯）→ 面向**调查人员 + 管理层**（用于取证和论证安全预算）；③ **QA & Compliance**（长期/治理）→ 面向**管理层、审计师、监管**（验证安全策略是否真的被执行）。

**17. 解释 False Positive、False Negative、Noise 的区别。为什么课件说 False Negative 最严重？误报失控又会导致什么？（→ §23）**

> **False Positive（误报）**：没有攻击却报警了。**False Negative（漏报）**：真有攻击却没报警。**Noise（噪音）**：报警**准确**但**没有威胁**（真实发生但不用管）。课件说漏报最严重，因为"undetected attack is far more dangerous than a false alarm"——攻击真的发生了却没人知道。误报失控会 desensitize（麻木）分析师，导致真正的警报也被忽略，**最终反而制造出漏报**。

**18. 三种检测方法的「参照物」分别是什么？各自的主要误差类型是什么？（→ §24）**

> **Signature-Based**：参照已知攻击特征库，主要误差是 **False Negative**（认不出新面孔）。**Anomaly-Based**：参照自己网络学出的正常基线，主要误差是 **False Positive**（容易冤枉正常行为）。**SPA/DPI**：参照厂商提供的协议规范，误差两者兼有，且可能影响业务可用性。

**19. Clipping Level 是什么？调紧和调松分别会导致什么后果？（→ §24）**

> Clipping Level 是 Anomaly-Based Detection 判断"偏离正常基线多少算异常"的**阈值/容忍线**。调**紧**（阈值低）：轻微波动就报警，**误报增多**；调**松**（阈值高）：只有很夸张的偏离才报警，**容易漏掉真正的攻击（漏报增多）**。

**20. Stateful Firewall 和 Stateful Protocol Analysis 有什么区别？（→ §24, §37.2）**

> **Stateful Firewall** 追踪的是**连接**是否存在（state table 记录 TCP 连接状态），判断"这个包属于已建立的连接吗"，属于**防火墙**范畴（p.19）。**Stateful Protocol Analysis** 追踪的是**协议会话**走到了哪个阶段（协议状态机），判断"这一步符合协议规范吗"，属于 **IDPS 的检测方法**（p.37）。两者都叫"stateful"但记的是完全不同的东西，不能混。

**21. Passive 和 Active IDPS 的区别是什么？Active 的四种响应动作是什么？（→ §25）**

> **Passive（=IDS）** 只分析报告、生成告警，不干预流量，等管理员处理；**Active（=IPS）** 触发告警时自动发起响应。Active 的四种响应动作：**终止用户会话/网络连接、封锁攻击源的访问、修改防火墙规则集、替换恶意内容**。IDS 加上自动响应能力就是 IPS，两者常是同一套软件，区别在于部署位置（旁路 vs 串接）和是否允许它动手。

**22. IDPS 的四条 Limitation 是什么？其中哪一条说明「IDPS 本身也是攻击目标」？（→ §26）**

> 四条：① 无法弥补底层安全机制本身的薄弱或缺失；② 无法检测尚未入库的新攻击或变种；③ 没有人类分析师介入无法自动调查事件；④ **无法完全抵御专门针对 IDPS 本身设计的规避或瘫痪攻击**——这一条说明 IDPS 本身也是攻击目标，攻击者会专门研究如何绕过或瘫痪它。

**23. 为什么说端点是「最弱的一环」？课件给的三个原因是什么？（→ §27）**

> 安全强度取决于最弱的一环，而端点通常最弱，原因三条：① **Number and variety**（数量多种类杂，管理面大、补丁难统一）；② **End users**（终端用户会点钓鱼链接、用弱密码）；③ **Privilege**（很多用户是本地管理员，账号被控就直接拿到高权限）。

**24. SIEM 的核心价值（一个词）是什么？为什么单个设备的告警往往看不出攻击？（→ §29）**

> 核心价值是 **Correlation（关联分析）**——把分散在不同系统里、单独看都无害的事件串成一条攻击链。单个设备的告警看不出攻击，是因为攻击链的每一步单独看都很正常（密码对了的登录、常见的提权操作、IT 常做的脚本下载、合法端口的流量），只有把时间线上跨系统的事件串起来才能看出这是一次数据窃取。

**25. SIEM 的五项 capabilities 是什么？（→ §29）**

> **Real-time monitoring**（实时监控）、**Incident response**（事件响应支撑）、**User monitoring**（用户行为监控）、**Threat intelligence**（威胁情报整合）、**Analytics and threat detection**（数据分析驱动的威胁检测）。

**26. NGFW 集成了哪五项能力？其中 Identity Awareness 为什么体现了 Zero Trust 思想？（→ §30）**

> 五项能力：**Firewall（基础包过滤）、IDS/IPS、Deep Packet Inspection、Identity Awareness、Threat Intelligence Integration**。Identity Awareness 把流量关联到具体的人/用户组，实现"基于身份"而不是"基于 IP 位置"的策略——这正是从"按位置授权"转向"按身份授权"，是 Zero Trust 思想在防火墙上的落地。

**27. Defense-in-Depth 定义里的三个形容词是什么？少一个为什么不行？（→ §31）**

> **Multiple（多重）、Independent（独立）、Overlapping（重叠）**。少一个都不行：只有多重没有独立，一个漏洞可能同时绕过所有层（机制相同）；只有独立没有重叠，会留下没人管的缝隙；没有多重，单层被绕过就直接失守。

**28. ZTA 落地的四个「Right」分别是什么？各自涉及哪个缩写？（→ §32）**

> **Right Users**（对的人）→ **PAM**（Privileged Access Management）；**Right Authentication**（对的验证）→ **MFA**；**Right Data**（对的数据）→ **DAM**（Data Activity Monitoring）；**Right Reason**（对的理由）→ 云访问审计、欺诈检测、配置管理（无专门缩写）。

**29. FIND / CONFIRM / FIX 三阶段各靠什么技术？课件给的三个痛点数据是什么、分别说明什么？（→ §33）**

> **FIND**（发现）靠 Real-Time Network Flow Analytics + UBA；**CONFIRM**（确认）靠 A.I. Triage 自动分诊验证；**FIX**（修复）靠 SOAR 自动化 Playbook。三个痛点数据：**1M+ daily feeds**（每天百万条情报，人处理不过来）、**20% intel indexed**（只有 20% 被真正索引利用，80% 是盲区）、**75% lack playbooks**（四分之三组织没有响应剧本）——三个数字层层递进，论证了为什么现代 SOC 必须靠 AI 和自动化。

**30. SIEM、XDR、SOAR、SOC 四者的区别？哪一个不是软件？（→ §34, §37.7）**

> **SIEM**：把所有日志收进来找异常（D）。**XDR**：跨端点/身份/邮件/云把攻击链串起来还能动手（D+R）。**SOAR**：发现之后按 playbook 自动处置（R）。**SOC**：谁来运营这一切——**SOC 不是软件，是组织**（人+流程+治理），这是四者里唯一不是软件/平台的。

**31. EDR 和 XDR 的区别？X 代表什么？（→ §34）**

> **EDR** 只看端点（Endpoint）；**XDR** 是 EDR 的扩展（eXtended），覆盖端点+身份+SaaS 应用+邮件协作+云+网络等多个面，能把 EDR 单独看不出的可疑行为跨面关联成完整攻击链，并自动采取行动。**X = eXtended（扩展）**。

**32. 蜜罐为什么误报率接近于零？这一点如何连接到 §23 的术语？（→ §35）**

> 蜜罐是一台没有任何正常业务、任何人都没理由访问的机器，所以**任何**对它的访问按定义就是可疑的——它不需要判断"这个行为像不像攻击"，只判断"有没有人碰了不该碰的东西"。连接到 §23 的术语：其他工具永远要在 False Positive 和 False Negative 之间权衡，蜜罐直接绕开了这个权衡，False Positive 天然接近于零。

**33. 蜜罐的三个设计目的是什么（英文动词）？（→ §35）**

> **Divert**（引开——把攻击者从真正的关键系统引开）、**Collect**（收集情报——观察攻击者的真实工具和手法/TTP）、**Delay**（拖延——留住攻击者，为管理员争取取证和响应的时间）。

**34. 蜜罐至少有哪三个潜在问题？（→ §35）**

> ① **法律与伦理风险**（entrapment 诱捕争议、隐私法、证据未必被法庭采纳）；② **被反向利用**——蜜罐本身是真机器，攻陷后可能被当跳板攻击内网其他系统或外部第三方，公司要承担下游责任；③ **老练攻击者能识别它**（anti-honeypot/fingerprinting 技术），识破后要么直接避开，要么故意喂假情报误导防御方向；（第 4、5 点：覆盖面有限，只能看到主动碰它的攻击者；成本与 ROI 难向管理层论证）。

**35. Summary 页的四条安全控制策略是什么？「lateral movement」是什么意思、由哪条策略对付？（→ §36）**

> 四条：**IAM - Least Privilege**（最小权限）、**Defense in Depth**（纵深防御）、**Fail-Safe Defaults / Zero-Trust**（默认拒绝）、**Simplicity and Auditability**（简洁性与可审计性）。**Lateral movement（横向移动）** 指攻击者攻陷一台低价值机器后，在内网里横向扩散寻找高价值目标；**Least Privilege** 策略通过只给完成本职工作所需的最小权限，掐断这条横向扩散的路径。
