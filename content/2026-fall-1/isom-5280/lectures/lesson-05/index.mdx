...
<Figure src="images/page_07.png" caption="How Generative AI works（用户 ↔ 模型直接问答） vs How AI agents work（用户交代目标，Agent 自己选工具、调数据库、执行）" source="Slide 7" />
...
<Callout type="exam" title="考点：这张三栏表本身就是一道「风险归类」送分题">
...
<Figure src="images/page_09.png" caption="Limitation I: Hallucinations, Injection & Reasoning——模型是概率型的模式匹配器，不是事实数据库" source="Slide 9" />
...
<Callout type="tip" title="小白类比：为什么幻觉这么难根治？">
...
<Figure src="images/page_13.png" caption="Real-World Costs of Ungoverned Shadow AI：医疗 $3.5M+ 罚款（用 ChatGPT 总结病历违反 HIPAA）、金融 $2.5M+ 罚款（借贷 AI 延续歧视性模式）、制造业 $54M+ 损失（专有代码通过 AI 编程助手训练数据泄露）" source="Slide 13" />
...
<Callout type="note" title="课务：Group Project（25%）简报">
...
<Figure src="images/page_25.png" caption="Core Concept：Threat（威胁）∩ Vulnerability（脆弱性）∩ Consequence（后果）三者的交集才构成 RISK" source="Slide 25" />
...
<Figure src="images/page_30.png" caption="NIST Cybersecurity Framework 2.0：Govern / Identify / Protect / Detect / Respond / Recover 六大 Function，每个再往下拆 Categories 和 Subcategories" source="Slide 30" />
...
<Figure src="images/page_35.png" caption="Threats-Vulnerabilities-Asset (TVA) 矩阵：每个「威胁 × 资产」交叉格填入具体的威胁-脆弱性组合，再按颜色标出处理的优先级" source="Slide 35" />
...
<Figure src="images/page_36.png" caption="Quantitative Risk Assessment：Loss Frequency = Attack Likelihood × Attack Success Probability；Loss Magnitude = Asset Value × Probable Loss；Calculate Risk = Loss Frequency × Loss Magnitude（± 不确定性）；再拿 Calculated Risk 和 Risk Appetite 比较" source="Slide 36" />
...
<Figure src="images/page_37.png" caption="Heat Map：Risk = Likelihood × Impact，5×5 矩阵映射到 Low / Medium / High / Critical 四个等级" source="Slide 37" />
...
<QA q="Applying controls and safeguards that eliminate or reduce the remaining uncontrolled risks is known as _____.（p.44 Quick Quiz）">
...
<QA q="Applying controls and safeguards that eliminate or reduce the remaining uncontrolled risks is known as _____.">
**Mitigation**。关键词"加控制/加防护措施去消除或降低剩余风险"直接对应 Mitigation 的定义。
</QA>

<QA q="一家公司决定关闭一个用了 15 年、几乎没人再用、但仍然连着内网的旧系统，而不是花钱升级它的安全防护。这属于四种风险处理策略里的哪一种？">
...
<QA q="为什么说 Agentic AI 的风险比 Generative AI 更高，即使它们都可能产生「幻觉」？">
...
<QA q="课件把 Loss Frequency 的不确定性设成 ±10%，最终算出 9~11 这个区间。如果 Risk Appetite 设为 10，这个风险应该 Accept 还是要 Treat？为什么用「区间上限」而不是「点估计」来判断更合理？">
