...
<QA q="Enterprise Data Model 和 Project Data Model 有什么区别？">
...
<QA q="为什么「系统生成的输出」（如报表）不能建成一个实体类型？">
...
<QA q="强实体、弱实体、关联实体分别怎么画？各自的标识符要求是什么？">
...
<QA q="判断一个候选实体是不是 associative entity，真正的判据是什么？「它跟两个实体都有关系」算不算判据？">
...
<QA q="Multivalued、Composite、Derived 三种属性分别怎么标记？为什么派生属性「标出来但不存」？">
...
<QA q="选标识符的三条规则是什么？为什么「有含义的编号」（intelligent identifier）是个坏主意？">
...
<QA q="判断基数约束的「两个问题法」是什么？「弱实体连 owner」「关联实体连两端实体」这两条白送规律分别怎么用？">
...
<QA q="Unary、Binary、Ternary 关系分别是什么？为什么三元关系不能拆成三个二元关系？">
...
<QA q="四轮扫描法的四轮分别找什么？为什么要分开扫，而不是边读边画？">
