...
*Physical Data Independence（Slide 28）*
...
*Relational Operations（Slide 29）*
...
*（网页版此处可以自己组合 σ / Π / ⋈；下面是课件的三张表和三个例子的结果）*

**Artist**

| ArtistID | ArtistName   | Year | Country |
| -------- | ------------ | ---- | ------- |
...
`Π Year, ArtistName (σ Year=1990 (Artist))`
...
`Π ArtistName (σ ReleaseYear=1994 (Artist ⋈ ArtistAlbum ⋈ Album))`
...
*（网页版此处是「这属于哪一层」的点选练习；下表是全部题目和答案）*

| 功能 / 概念                                     | 属于                             | 理由                                                              |
| ------------------------------------------- | ------------------------------ | --------------------------------------------------------------- |
| 写 SQL 语句、发送 SQL commands                    | **SQL Client**                 | SQL Client 是发出 SQL commands 的地方                                 |
| 检查 SQL 语法、验证表名 / 列名是否存在                     | **Query Parsing & Planning**   | parse → check syntax → validate table and column references     |
| 生成 logical query plan（σ Π ⋈ γ τ 树）          | **Query Parsing & Planning**   | 解析之后产出 logical plan                                             |
| 在多个 physical plan 中挑更快的（查询优化）               | **Query Parsing & Planning**   | p.38 这一层就叫 Query Parsing & Optimization                         |
| 执行 Hash Join / Index-Nested Loop Join       | **Operator Execution**         | 具体算法由执行层（Relational Operators）运行                                |
| 执行 External Merge-Sort、Hash-based Aggregate | **Operator Execution**         | physical plan 里的算法都在 Operator Execution 跑                       |
| 把一张表组织成 fixed-size pages 的文件                | **Files and Index Management** | A relation is treated as a file of fixed-size pages             |
| Slotted page、slot array                     | **Files and Index Management** | page 内部布局归 Files and Index Management                           |
| Record ID = (File ID, Page ID, Slot #)      | **Files and Index Management** | 记录的物理地址由文件 / 页组织决定                                              |
| 用索引（B+ tree）把 key 映射到 record ID             | **Files and Index Management** | Index 属于 Files and Index Management，不属于 Buffer 或 Disk           |
| Buffer pool、frames、在内存中修改 page              | **Buffer Management**          | 内存里的 page 由 buffer manager 管                                    |
| 高层要一个 page，请求交给谁                            | **Buffer Management**          | Higher layers request pages through the buffer manager          |
| 把 page 读 / 写到物理磁盘或文件                        | **Disk Management**            | 磁盘上的 page 由 disk manager 管                                      |
| 多个用户同时改数据也不出错                               | **Cross-cutting（不是一层）**        | Concurrency control 是 cross-cutting issue，跨 storage 与 memory 管理 |
| 系统崩溃后把数据恢复到一致状态                             | **Cross-cutting（不是一层）**        | Recovery 同样是 cross-cutting，不是单独一层                               |
...
*（网页版此处是可操作的 buffer pool，3 个 frame；下表是示例步骤，\* = dirty）*

| # | 请求             | 发生了什么                                                                                                      | 之后的 frames |
| - | -------------- | ---------------------------------------------------------------------------------------------------------- | ---------- |
| 1 | Get Page #2    | 未命中 (miss)：page 2 不在内存；disk manager 从 database file 读出 page 2，放进空 frame                                    | 2          |
| 2 | Update Page #2 | 命中 (hit)：page 2 已在 buffer pool，不用读磁盘；在内存副本上修改 page 2 → 标记为 dirty（还没写回磁盘）                                   | 2\*        |
| 3 | Get Page #5    | 未命中 (miss)：page 5 不在内存；disk manager 从 database file 读出 page 5，放进空 frame                                    | 2\*, 5     |
| 4 | Get Page #1    | 未命中 (miss)：page 1 不在内存；disk manager 从 database file 读出 page 1，放进空 frame                                    | 2\*, 5, 1  |
| 5 | Get Page #6    | 未命中 (miss)：page 6 不在内存；frame 满了，要踢出 page 2；它是 dirty，先写回磁盘；disk manager 从 database file 读出 page 6，放进空 frame | 5, 1, 6    |
| 6 | Get Page #5    | 命中 (hit)：page 5 已在 buffer pool，不用读磁盘                                                                       | 5, 1, 6    |
| 7 | Update Page #5 | 命中 (hit)：page 5 已在 buffer pool，不用读磁盘；在内存副本上修改 page 5 → 标记为 dirty（还没写回磁盘）                                   | 5\*, 1, 6  |
| 8 | Flush          | 把 page 5 写回磁盘（dirty → clean）                                                                               | 5, 1, 6    |
...
- A. SQL Client
- B. Query Parsing & Planning
- C. Files and Index Management
- D. Disk Management
...
**3. Choosing between a Hash Join and an Index-Nested Loop Join is a decision about the \_\_\_ plan.**
...
- A. reading the disk directly
- B. requesting them through the buffer manager
- C. asking the SQL client
- D. scanning the index
...
- A. the top layer
- B. part of Query Parsing
- C. cross-cutting issues related to storage and memory management
- D. part of the SQL Client
...
**8. “Each layer abstracts the layer \_\_\_.“ Fill in the blank.**
...
- A. Logical tells what is computed
- B. Physical tells how it is computed
- C. One logical plan maps to exactly one physical plan
- D. It enables behind-the-scenes optimizations
...
- A. only the Year column
- B. only rows where Year = 1990
- C. all rows sorted by Year
- D. a join of Artist with Year
...
- A. They make inserts faster
- B. They map key values to record IDs
- C. They sort the table physically
- D. They have no maintenance cost
...
**16. SQL is described in the lecture as a \_\_\_ language.**
...
- A. page 2 on disk directly
- B. the in-memory copy of page 2 in the buffer pool
- C. the directory only
- D. the SQL client cache
