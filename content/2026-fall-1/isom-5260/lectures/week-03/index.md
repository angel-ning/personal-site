...
> **踩坑提醒**：一个子类如果既没有独有属性、也没有独有关系，就**不应该**画成子类——见 §5 的 Motorcycle。
...
> **考点**：给一段业务描述，问某个关系应该连在超类还是子类上。判断标准：**是不是所有成员都有这个关系**。
...
**记忆锚点**：拆超类/子类 = **消灭"只对部分行有意义"的列所产生的 null**。
...
> **小白类比**：Generalization 是"把几个抽屉里相同的东西拿出来放进一个公共柜子"；Specialization 是"发现一个柜子里有些东西只有部分人用，于是分出小抽屉"。
...
> **踩坑提醒**：Q2 是 **Yes → o、No → d**。"能同时属于多个吗？能 = overlap"。别被 "disjoint" 这个词带偏以为 Yes 对应 d。
...
**记忆口诀**：**线数管"有没有"，字母管"多不多"**。双线 = 必有一个；o = 可以多个。
...
> **小白理解**：d 就像单选题，填一个选项字母（"O" 或 "R"）就够了；o 就像多选题，要给每个选项一个勾（Manufactured? = Y、Purchased? = Y），所以 discriminator 必须拆成多个 Y/N 组成部分。P3055 Circuit Board 两个都是 Y，就是"两个子类都属于"。
...
> **踩坑提醒**（常见扣分）：圆圈写了 **o** 却给了一个 simple 的 `Part Type = "M"/"P"`——那 Circuit Board 这种两个都是的零件就没法表示，**图和 discriminator 自相矛盾**。反过来，圆圈写 **d** 却用了 Y/N composite，意味着允许两个都是 Y，也和 d 矛盾。

> **🔴 补充规则（课件没有明写，由两个约束直接推出）**：线数也会限制 discriminator 的取值——**双线（total）时不应出现"不指向任何子类"的值**（d：不能有 O/R 以外的值或空值；o：不能有全是 N 的行）；单线（partial）时则允许，比如 VEHICLE 里摩托车的判别值不对应任何子类。
...
> **考点**：问 GRADUATE STUDENT 有哪些属性 → 自己的 Test Score + STUDENT 的 Major Dept + PERSON 的 SSN、Name、Address、Gender、Date Of Birth（一路继承到 root）。
...
**Q2**：MOTORCYCLE 没有独有属性（*each subtype must have unique attributes, unique relationships, or both*）。存在不属于任何子类的车 → **partial（单线）**。

**Q3**：Q1 Yes → 双线；Q2 No → d。**双线 + d**。

**Q4**：Q1 No → 单线；Q2 Yes → o。**单线 + o**。

**Q5**：Composite，例如 `Employee Type (Manager?, Engineer?)`，连线标签 `Manager? = "Y"`、`Engineer? = "Y"`。因为是 partial，允许出现 (N, N) 的员工。

**Q6**：o 表示一个零件可以同时是两种，simple 的单一取值无法表示"两个都是"，和 o 矛盾。改为 composite：`Part Type (Manufactured?, Purchased?)`，连线标 `Manufactured? = "Y"` / `Purchased? = "Y"`。

**Q9**：Class Standing（自己）+ Major Dept（STUDENT）+ SSN、Name、Address、Gender、Date Of Birth（PERSON）。

**Q10**：`Account Type = "S" / "C"`，ACCOUNT 方框里要有 Account Type，两条连线分别标 "S"、"C"；数据表 AccountType 列只能出现 S 或 C，不能为空（total）。
