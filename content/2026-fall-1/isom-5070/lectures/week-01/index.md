...
1. 请解释 Prevention、Mitigation、Remediation 三者的区别，并各举一个网络安全的例子。
2. 列举风险评估的5个主维度（Primary Dimensions）并各用一句话解释。
3. 为什么说"Risks DO NOT have a single consequence"？请结合CrowdStrike 2024案例说明。
4. 比较中国、欧盟、美国网络安全法律监管的核心差异（至少从3个维度）。
5. GDPR、NIS2、DORA三者分别侧重什么？
6. 解释CIA+2（五性）分别代表什么，并说明InfoSec的四大关注领域。
7. 为什么说Cyber Security Risk应该被纳入企业整体的ERM框架，而不只是IT部门的问题？请列举至少3个治理角色及其职责。
8. 中国的CSL、DSL、PIPL三部法律分别管什么？它们之间是什么关系？
9. 香港新颁布的PCICSO（关键基础设施保护条例）何时生效？适用于哪些行业？
10. 什么是Attack Surface？请列举其五大类别。
