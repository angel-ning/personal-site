...
<Callout type="tip" title="为什么单独列这张表">
...
<Callout type="board" title="课件例句：把五要素串成一句话（可以直接当模板背）">
...
<Callout type="extra" title="课外补充：这张清单其实是在说「情报不是凭空想象」">
...
<Figure src="images/RISK_MATRIX.png" caption="5×5 风险矩阵：横轴 Impact（后果严重度），纵轴 Probability（发生可能性），交叉格 = 两者相乘的风险分数" source="Slide 11" />
...
<Figure src="images/FAIR_TREE.png" caption="FAIR 因子分解树：Risk 一路拆到最底层的 6 个可估计输入" source="Slide 23" />
...
<Callout type="exam" title="怎么把这 6 项分成 Primary / Secondary（考试可能反着问）">
...
<Figure src="images/MONTE_CARLO.png" caption="每个输入（Contact Frequency、Probability of Action、Threat Capability、Resistance Strength…）都不是一个单点数字，而是一条概率分布；层层组合算出 LEF、Vulnerability 和 Secondary Loss Frequency 各自的分布" source="Slide 33" />
...
<QA q="按顺序写出 Cyber Security Risk Management 七步法的名称（不要求一字不差，但顺序和意思要对）">
...
<QA q="一个合格的威胁模型 (threat model) 必须包含哪五个要素？请用一个场景把它们串起来。">
...
<QA q="Inherent Risk 和 Residual Risk 的区别是什么？为什么风险管理的目标从来不是把风险降到零？">
...
<QA q="给定 Baseline expected loss = US$6.0M，MFA+PAM 处置后 residual expected loss = US$2.2M，控制成本（一次性+年度）= US$2.2M。请计算 Expected Benefit、Net Benefit 和这笔投资值不值得做。">
...
<QA q="给定 TEF = 1.0 次/年，Vulnerability = 25%，Primary loss = US$5.3M，Secondary loss = US$4.0M，用 FAIR 模型算出年化预期损失 (Risk)。">
...
<QA q="什么时候不应该为一个风险跑完整的 FAIR 定量模型？列举至少三种情况。">
...
<QA q="Monitor、Test、Report、Reassess 这四个动作分别回答什么问题？">
